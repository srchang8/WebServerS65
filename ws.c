...
void send_server_info(FILE *fp);
void do_head(char *dir, FILE *fp);
...
main(int ac, char *av[])
{
	...
	/* set up 
		create socket, assign an address, 
		setup to receive incoming calls
		use conf? to assign myhost and myport
	*/
	sock = startup(ac, av, myhost, &myport);

	/* print host and port after starting server */
...
int startup(int ac, char *av[],char host[], int *portnump)
{
	...
	//check if there is command line args with -c
	//if there is, point configfile to it, else use default: wsng.conf
...
/* ------------------------------------------------------ *
   the reply er thing: all functions need one
...
/* ------------------------------------------------------ *
  ...
  sends back time/date, server name and version
   ------------------------------------------------------ */

void
send_server_info(FILE *fp)
{
	//send out date
	time_t timer;
    char buffer[42];
    ...
    strftime(buffer, 42, "%Y-%m-%d %H:%M:%S", tm_info);
    fprintf(fp, "Time and Date: %s\n", buffer);

	//send out server name and version
	char hostname[1024];
	hostname[1023] = '\0';
	...
	struct utsname serverVersion;
	uname(&serverVersion);
	fprintf(fp, "Server version: %s-%s\n", serverVersion.sysname, serverVersion.release);

}

/* ------------------------------------------------------ *
  Send back just the header without contents
  etc: HEAD nameOfFile httpVersion
   ------------------------------------------------------ */

void
do_head(char *dir, FILE *fp)
{
	header(fp, 200, "OK", "text/plain");
}
